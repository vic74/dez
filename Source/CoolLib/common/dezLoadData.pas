...
begin
  ...
  begin
    ...
    SQ1 := StrToFloatDef(cmCorrectFloat(cmGetWord(S, 3, Rzd)), 0);
    SQ2 := StrToFloatDef(cmCorrectFloat(cmGetWord(S, 4, Rzd)), 0);
    Summa := StrToFloatDef(cmCorrectFloat(cmGetWord(S, 5, Rzd)), 0);
    if Summa <> 0 then
      with AddShb do
      begin
        ParamByName('Code').AsInteger := fMakeSmetaCode;
        ParamByName('House').AsInteger := House;
        ParamByName('SQ1').AsCurrency := SQ1;
        ParamByName('SQ2').AsCurrency := SQ2;
...
begin
  ...
  AddShb := cmDB.GetQuery
    ('insert into MakeSmetaShablonSpec(MakeSmetaShablon, House, Summa, SQ1, SQ2) ' +
    'values(:Code, :House, :Summa, :SQ1, :SQ2)', cmDB.WT);
end;
